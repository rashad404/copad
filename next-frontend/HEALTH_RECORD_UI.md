...
- `node --test tests/health-record.test.mjs tests/health-record-dom.test.cjs`: 13 passing tests, including DOM interactions with mocked authenticated APIs, cross-member response isolation, read-only controls, original units, critical flags, and failed saves.
...
Run the isolated worktree frontend on port 3005. Its ignored `.env.local` uses:
...
Preview: `http://100.89.150.50:3005/health-record` (sign in required).
