...
public class OAuth2Config {
    ...
    /**
     * Where Google should send the person back to.
     *
     * Taken from the request, so somebody who starts at azdoc.ai finishes
     * there. This read X-Forwarded-Host before, which ForwardedHeaderFilter has
     * already consumed and removed, so it always fell through to a hardcoded
     * virtualhekim.az.
     */
    private String determineBaseUrl(HttpServletRequest request, String origin, String referer,
        ...
        String baseUrl = SiteOrigin.of(request);
        logger.info("OAuth2 base URL resolved to {}", baseUrl);
        return baseUrl;
    }
} 